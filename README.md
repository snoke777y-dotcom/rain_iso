...
- 连续色带栅格渲染与 BMP 导出
- 浏览器样例壳、时间轴回放与单页自适应画布
- 回归测试、接口测试、导入脚本测试

最近一轮已落地的浏览器侧优化：

- 时间轴播放支持 `1x / 2x / 3x`
- 时间轴切换改为按需 `setTimeout` 调度，拖动和切帧不再被固定间隔阻塞
- 结果画布按容器同比例缩放，默认保持单页显示，不再依赖页面滚动
- 计算 Worker 与渲染 Worker 分离，渲染结果支持预热缓存
- 单任务级站点元数据缓存已前移，避免每帧重复构建索引
- `render-grid-layer` 已把格点字符串 `Map` 查找改为定长数组查表，并去掉主像素循环里的重复图例分派
...
基于仓库内置样例与 CPU 路径，最近一次离线复测结果如下：

- 首帧计算：约 `40.5ms`
- 首帧渲染：约 `677.2ms`
- 整任务 `195` 帧总耗时：约 `10.48s`

这组数据来自本地离线脚本复测，不是浏览器可见上屏时间；它能证明当前总计算时长没有因为最近一轮首帧优化而回退，同时渲染热区已经从秒级明显下降。
