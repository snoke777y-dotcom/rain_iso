...
import type { BackendKind, FrameResult, FrameType } from "../../../domain/rain_iso/models.js";
...
export function buildFrameResult(options: {
  ...
  const maxValue =
    renderableValues.length > 0 ? Math.max(...renderableValues) : 0;
  const minRenderableValue =
    renderableValues.length > 0 ? Math.min(...renderableValues) : undefined;
  const meanRenderableValue =
    renderableValues.length > 0
...
function countMarked(mask: Uint8Array): number {
  let count = 0;
