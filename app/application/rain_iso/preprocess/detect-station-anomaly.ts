...
export function detectStationAnomaly(
  observation: StationObservation,
  options: {
    frameType: FrameType;
    allValidStations: StationObservation[];
    stationById?: ReadonlyMap<string, StationObservation>;
    fallbackNeighborStationIdsByStationId?: ReadonlyMap<string, string[]>;
  }
): AnomalyDetectionResult {
  const neighborhoodStations = options.allValidStations.filter(
    (candidate) =>
      candidate.stationId !== observation.stationId &&
      getDistanceMeters(observation, candidate) <= 5_000
  );
  const supplementedNeighborhoodStations = supplementNeighborhoodStations(
    observation,
    neighborhoodStations,
    {
      ...
      fallbackNeighborStationIdsByStationId:
        options.fallbackNeighborStationIdsByStationId
    }
  );
...
function supplementNeighborhoodStations(
  observation: StationObservation,
  neighborhoodStations: StationObservation[],
  options: {
    stationById?: ReadonlyMap<string, StationObservation>;
    fallbackNeighborStationIdsByStationId?: ReadonlyMap<string, string[]>;
  }
  ...
  const fallbackNeighborStationIds =
    options.fallbackNeighborStationIdsByStationId?.get(observation.stationId);
  ...
  const seenStationIds = new Set(
    supplementedStations.map((station) => station.stationId)
  );
...
function resolveLegendBinIndex(
  bins: Array<{
    min: number;
    max: number | null;
  }>,
...
function getDistanceMeters(
  left: Pick<StationObservation, "longitude" | "latitude">,
  right: Pick<StationObservation, "longitude" | "latitude">
): number {
