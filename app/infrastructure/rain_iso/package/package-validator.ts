...
export class PackageValidationError extends Error {
  public readonly code = "PACKAGE_VALIDATION_FAILED";
...
export function validateRawRainApiResponse(
  response: RawRainApiResponse,
  options: {
    expectedProductType: FrameType;
  }
): void {
  if (response.code !== "0") {
    throw new PackageValidationError(
      `${options.expectedProductType} response code must be 0`
    );
  }
  ...
  if (timeKeys.length === 0) {
    throw new PackageValidationError(
      `${options.expectedProductType} response must contain at least one frame`
    );
  }

  const baseStationIds = new Set(
    (response.data[timeKeys[0]] ?? []).map((record) => record.stcd)
  );
  if (baseStationIds.size === 0) {
    throw new PackageValidationError(
      `${options.expectedProductType} response must contain at least one station`
    );
  }

  for (const timeKey of timeKeys.slice(1)) {
    const stationIds = new Set(
      (response.data[timeKey] ?? []).map((record) => record.stcd)
    );
    if (stationIds.size !== baseStationIds.size) {
      throw new PackageValidationError(
        `${options.expectedProductType} station count mismatch across frames`
      );
    }
    ...
    for (const stationId of baseStationIds) {
      if (!stationIds.has(stationId)) {
        throw new PackageValidationError(
          `${options.expectedProductType} station set mismatch across frames`
        );
      }
    }
  }
...
export function validateLoadedRainIsoPackage(
  dataPackage: {
    stationIds: string[];
...
export function validateRawPackage(rawPackage: {
  realtime5mResponse: RawRainApiResponse;
  realtime1hResponse: RawRainApiResponse;
}): void {
