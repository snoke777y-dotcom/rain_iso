const Be = "1.0.0", De = "rain-iso-profile-v1";
...
function We(e, a) {
  const r = e.filter(
    (u) => a.fixedAnchorStationIds.has(u.stationId)
  ), t = new Set(
    r.map((u) => u.stationId)
  ), n = ze(
    e.filter((u) => !t.has(u.stationId))
  ), s = /* @__PURE__ */ new Map();
  for (const u of [...r, ...n])
    s.has(u.stationId) || s.set(u.stationId, u);
  const i = Array.from(s.values()), o = new Set(i.map((u) => u.stationId)), l = e.filter(
    (u) => !o.has(u.stationId)
  );
  return {
...
const N = {
  ...
  [N.CoreGuard]: Y.Highest,
  [N.TongzhouGuard]: Y.Highest,
  [N.CrossBorderChannel]: Y.Highest,
  [N.DynamicTop30]: Y.Normal,
  [N.OrdinaryStation]: Y.None
};
function ge(e) {
  return He[e];
}
...
function q(e, a) {
  ...
  return a.stationMeta.stations.forEach((t, n) => {
    r.set(String(t.station_id), a.stationToGrid[n]);
  }), e.flatMap((t) => {
...
function Je(e, a) {
  const r = a.assets.manifest.grid_count, t = new Float32Array(r), n = new Uint8Array(r), s = new Uint8Array(r), i = q(e.fixedAnchorStations, a.assets).map(
    (d) => ({
      ...d,
      kind: N.CoreGuard
    })
    ...
    ...d,
    kind: N.DynamicTop30
  })), l = q(e.ordinaryStations, a.assets), u = /* @__PURE__ */ new Map();
  for (const d of [...i, ...o]) {
    const f = u.get(d.gridId);
    if (!f) {
      u.set(d.gridId, d);
      continue;
    }
    ...
    u.set(
      d.gridId,
      k.stationId === f.stationId ? f : d
    );
  }
  for (const [d, f] of u)
    t[d] = f.value, n[d] = 1;
  const g = u.size === 0, c = /* @__PURE__ */ new Map();
  for (const d of l) {
    const f = c.get(d.gridId);
    f ? f.push(d) : c.set(d.gridId, [d]);
  }
  for (const [d, f] of c) {
    if (u.has(d))
      continue;
    const k = qe(f, {
      referenceValue: g ? null : Qe(d, u, t, a.assets)
    });
    t[d] = k.value, s[d] = 1;
  }
  return {
...
function Qe(e, a, r, t) {
  ...
  for (const i of a.keys()) {
    const o = t.gridMeta.centerX[i] - t.gridMeta.centerX[e], l = t.gridMeta.centerY[i] - t.gridMeta.centerY[e], u = o * o + l * l;
    (u < s || u === s && n !== null && r[i] > r[n]) && (n = i, s = u);
  }
...
function ta(e) {
  ...
  const l = e.radiusConfig?.hardAnchorBonus ?? 1, u = new Int32Array(e.gridMask.length).fill(-1), g = [], c = [];
  let d = 0;
  for (const f of r)
    g.push(f), c.push(i + l);
  for (const f of t)
  ...
  for (; d < g.length; ) {
    const f = g[d], k = c[d];
    if (d += 1, f < 0 || e.gridMask[f] !== 1 || k <= u[f] || (u[f] = k, o[f] = 1, k === 0))
      continue;
    const I = f * 8;
    for (let x = 0; x < 8; x += 1) {
      const y = e.gridNeighbors[I + x];
      y < 0 || (g.push(y), c.push(k - 1));
    }
  }
  return {
    knownMask: a,
...
const Re = {
  legendId: xe.Legend5mV1,
  productType: D.Rain5m,
  bins: [
    C(0.1, 0.4, "#97F297", "#333333"),
    C(0.4, 1, "#3DCE3D", "#333333"),
    C(1, 2, "#6ACEF2", "#333333"),
    C(2, 5, "#1010F2", "#ffffff"),
    C(5, 10, "#F210F2", "#ffffff"),
    C(10, 15, "#A0103D", "#ffffff"),
    C(15, 20, "#f8aa0a", "#ffffff"),
    C(20, null, "#9933FF", "#ffffff")
  ]
}, Ee = {
  legendId: xe.LegendAccum24hV1,
  productType: D.Accum1hStep,
  bins: [
    C(0.1, 10, "#97F297", "#333333"),
    C(10, 25, "#3DCE3D", "#333333"),
    C(25, 50, "#6ACEF2", "#333333"),
    C(50, 100, "#1010F2", "#ffffff"),
    C(100, 250, "#A0103D", "#ffffff"),
    C(250, 400, "#f8aa0a", "#ffffff"),
    C(400, null, "#9933FF", "#ffffff")
  ]
}, na = {
  [D.Rain5m]: Re,
  [D.Accum1hStep]: Ee
...
function sa(e, a) {
  const r = a.allValidStations.filter(
    (c) => c.stationId !== e.stationId && oa(e, c) <= 5e3
  ), t = ia(
    e,
    r,
    {
      stationById: a.stationById ?? new Map(a.allValidStations.map((c) => [c.stationId, c])),
  ...
  const n = t.map((c) => c.value).sort((c, d) => d - c), s = n[0], i = n[1];
  if (i === void 0 || s > i * 2)
    return {
      ...e,
      status: "normal",
  ...
  const o = Oe(a.frameType), l = he(o.bins, s), u = he(o.bins, e.value), g = Math.abs(u - l);
  return g <= 1 ? {
    ...e,
    status: "normal",
    canBeDynamicAnchor: !0
...
function ia(e, a, r) {
  ...
  for (const o of t) {
    if (o === e.stationId || i.has(o))
      continue;
    const l = n.get(o);
    if (l && (s.push(l), i.add(o), s.length >= 4))
      break;
  }
  return s;
}
...
function da(e, a) {
  ...
  }), n = [...t], s = r.filter((l) => l.value === 0 ? (n.push({
    ...l,
    reason: "zero_rain_filtered"
  }), !1) : !0), i = [], o = new Map(
    s.map((l) => [l.stationId, l])
  );
  for (const l of s) {
    const u = sa(l, {
...
function se(e) {
  ...
  for (let s = 0; s < a; s += 1) {
    ...
    for (let o = 0; o < n.length; o += 1) {
      ...
      const l = [n[o]], u = o * 8;
      for (let c = 0; c < 8; c += 1) {
        const d = e.gridNeighbors[u + c];
        d < 0 || e.rainMask[d] !== 1 || l.push(n[d]);
      }
      const g = l.reduce((c, d) => c + d, 0) / l.length;
...
function ie(e) {
  ...
  for (; n; ) {
    ...
    for (let s = 0; s < t; s += 1) {
      ...
      const i = [], o = [];
      for (let u = 0; u < 8; u += 1) {
        const g = e.gridNeighbors[s * 8 + u];
        if (g < 0 || r[g] === 0)
          continue;
        (e.hardAnchorMask[g] === 1 ? o : i).push(a[g]);
      }
      const l = i.length > 0 ? i : o;
      l.length !== 0 && (a[s] = ga(l), r[s] = 1, n = !0);
    }
  }
  return {
    valueGrid: a,
  ...
  for (; ; ) {
    const T = r.createBindGroup({
      ...
      entries: [
        ...
        { binding: 5, resource: { buffer: f } },
        { binding: 6, resource: { buffer: k } },
        { binding: 7, resource: { buffer: I } },
        { binding: 8, resource: { buffer: x } }
      ]
    }), A = r.createCommandEncoder(), _ = A.beginComputePass();
    _.setPipeline(y), _.setBindGroup(0, T), _.dispatchWorkgroups(Math.ceil(t / 64)), _.end(), r.queue.submit([A.finish()]);
    const V = await _e(
      r,
      I,
      t,
    ...
    if (!_a(V)) {
      ...
      return { valueGrid: v, knownMask: E };
    }
    [l, g] = [g, l], [u, c] = [c, u];
  }
}
...
async function ya(e) {
  ...
  let l = M({
    ...
    size: Math.max(4, G(i)),
    initialData: e.valueGrid,
    usage: b.STORAGE | b.COPY_DST | b.COPY_SRC
  }), u = M({
    device: r,
  ...
  for (let y = 0; y < t; y += 1) {
    ...
    _.setPipeline(x), _.setBindGroup(0, T), _.dispatchWorkgroups(Math.ceil(s / 64)), _.end(), r.queue.submit([A.finish()]), [l, u] = [u, l];
  }
  return Ta(r, l, s, `smooth:readback:${o}`);
}
async function Ta(e, a, r, t) {
  ...
  s.copyBufferToBuffer(a, 0, n, 0, r * 4), e.queue.submit([s.finish()]), await n.mapAsync?.(Ma);
  const i = n.getMappedRange?.() ?? new ArrayBuffer(r * 4), o = new Float32Array(i.slice(0));
  return n.unmap?.(), o;
}
function G(e) {
...
function oe(e) {
  ...
  try {
    const n = Sa(t), s = {
      ...
      programs: {
        [p.ContinuousPropagate]: ye(
          t,
          p.ContinuousPropagate,
          n
        ),
        [p.ConstrainedSmooth]: ye(
          t,
          p.ConstrainedSmooth,
          n
        )
...
function Sa(e) {
  ...
  if (e.shaderSource(a, va), e.compileShader(a), !e.getShaderParameter(a, e.COMPILE_STATUS ?? h.COMPILE_STATUS))
...
async function Ra(e) {
  ...
  try {
    ...
    for (let T = 0; T < t; T += 1) {
      ...
      for (let _ = 0; _ < 4; _ += 1)
        i[A + _] = e.gridNeighbors[T * 8 + _], o[A + _] = e.gridNeighbors[T * 8 + _ + 4];
    }
    const l = `propagate:stateA:${t}`, u = `propagate:stateB:${t}`;
    let g = !0, c = S({
      runtime: a,
      key: l,
      width: t,
    ...
    for (; Fe({
      ...
      textures: [
        { texture: c, unit: 0, uniformName: "uStateTex" },
        { texture: f, unit: 1, uniformName: "uMaskTex" },
        { texture: k, unit: 2, uniformName: "uNeighborTexA" },
        { texture: I, unit: 3, uniformName: "uNeighborTexB" }
      ]
    }), y = Ne(a, t), !!Ea(y); ) {
      const A = y;
      c = d, g = !g, d = S({
...
async function wa(e) {
  ...
  if (!a)
    return se(e);
  try {
    const { gl: r } = a, t = e.valueGrid.length, n = e.rounds ?? te, s = e.softObsMaxDelta ?? ne, i = new Float32Array(t * 4), o = new Float32Array(t * 4), l = new Float32Array(t * 4), u = new Float32Array(t * 4), g = new Float32Array(t * 4);
    for (let v = 0; v < t; v += 1) {
      const E = v * 4;
      i[E] = e.valueGrid[v], o[E] = e.valueGrid[v], l[E] = e.rainMask[v], l[E + 1] = e.hardAnchorMask[v], l[E + 2] = e.softObsMask[v];
      for (let P = 0; P < 4; P += 1)
        u[E + P] = e.gridNeighbors[v * 8 + P], g[E + P] = e.gridNeighbors[v * 8 + P + 4];
    }
    const c = `smooth:valueA:${t}`, d = `smooth:valueB:${t}`;
    let f = !0, k = S({
      runtime: a,
      key: c,
      width: t,
    ...
    for (let v = 0; v < n; v += 1)
      Fe({
        ...
        textures: [
          { texture: k, unit: 0, uniformName: "uValueTex" },
          { texture: x, unit: 1, uniformName: "uOriginalTex" },
          { texture: y, unit: 2, uniformName: "uMaskTex" },
          { texture: T, unit: 3, uniformName: "uNeighborTexA" },
          { texture: A, unit: 4, uniformName: "uNeighborTexB" }
        ]
      }), V = Ne(a, t), k = I, f = !f, I = S({
        runtime: a,
        key: f ? d : c,
        width: t,
        pixels: V
...
async function Ga(e, a) {
  const r = la(e, a.assets), { effectiveStations: t } = da(r, {
    ...
    hardAnchorMask: s.hardAnchorMask,
    softObsMask: s.softObsMask,
    radiusConfig: a.rainMaskRadiusConfig
  }), { propagated: o, smoothedValueGrid: l, actualBackend: u } = await Na({
    selectedBackend: a.selectedBackend,
    valueGrid: s.valueGrid,
    rainMask: i.rainMask,
    knownMask: i.knownMask,
    hardAnchorMask: s.hardAnchorMask,
    softObsMask: s.softObsMask,
    gridNeighbors: a.assets.gridNeighbors,
    gridCenterX: a.assets.gridMeta.centerX,
    gridCenterY: a.assets.gridMeta.centerY,
    ordinaryOnlyMode: s.ordinaryOnlyMode,
  ...
  if (u !== "cpu" && Fa({
    sourceValueGrid: s.valueGrid,
    sourceRainMask: i.rainMask,
    resultValueGrid: c.valueGrid,
    resultRainMask: c.rainMask
  })) {
    const f = await z({
      valueGrid: s.valueGrid,
      rainMask: i.rainMask,
  ...
  return ca({
    ...
    frameTime: e.frameTime,
    selectedBackend: g,
    valueGrid: c.valueGrid,
    rainMask: c.rainMask,
    hardAnchorMask: s.hardAnchorMask,
    softObsMask: s.softObsMask,
    knownMask: o.knownMask,
    suspectStationCount: d,
    ordinaryOnlyMode: s.ordinaryOnlyMode
  });
}
function Fa(e) {
...
function ve(e) {
  const a = Ba(e.frameTimes), r = e.stationIds.length;
...
async function Va(e) {
  ...
  for (const u of t) {
    ...
    const g = a(), c = await Ga(u, {
      assets: e.assets,
      selectedBackend: l,
      rainMaskRadiusConfig: e.rainMaskRadiusConfig
    });
    l = c.selectedBackend, c.summary.elapsedMs = a() - g, i.recordFrame(c.summary.elapsedMs), o += 1, e.onFrameReady?.({
      taskId: e.taskId,
      frameKey: u.frameKey,
      frameResult: c
    }), e.onTaskProgress?.({
...
function Ya(e) {
  if (e.asset_manifest.protocol_version !== Be)
...
function Ka(e) {
  return e instanceof R ? {
    code: B.AssetValidationFailed,
    message: e.message
  } : e instanceof H ? {
    code: B.PackageValidationFailed,
    message: e.message
  } : e instanceof Error && "code" in e && typeof e.code == "string" ? {
    code: e.code,
    message: e.message
  } : e instanceof Error ? {
    code: B.FrameComputeFailed,
    message: e.message
  } : {
    code: B.UnknownError,
