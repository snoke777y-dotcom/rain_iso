...
describe("cell ordinary selection", () => {
  it("同格多个普通站时按最近活动锚点值择优", () => {
    ...
    const anchorSets = createAnchorSets({
      ...
      dynamic: [],
      ordinary: [station("ORD-1", 8), station("ORD-2", 20)]
    });

    const result = buildSeedGrid(anchorSets, { assets });

    expect(result.valueGrid[1]).toBe(8);
    expect(result.softObsMask[1]).toBe(1);
    expect(result.ordinaryOnlyMode).toBe(false);
  });

  it("本帧无活动锚点时同格普通站取最大值并标记 ordinary_only_mode", () => {
    const assets = createAssets({
      gridCount: 2,
      stationIds: ["ORD-1", "ORD-2"],
      stationToGrid: [1, 1]
    });
    const anchorSets = createAnchorSets({
      fixed: [],
    ...
    expect(result.valueGrid[1]).toBe(20);
    expect(result.softObsMask[1]).toBe(1);
