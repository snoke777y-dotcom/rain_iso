...
export function createFrameViewModel(options) {
    const orderedFrames = [...options.frames].sort((left, right) => left.frameTime.localeCompare(right.frameTime) ||
        left.frameKey.localeCompare(right.frameKey));
    const renderedByKey = new Map(orderedFrames.map((frame) => [
        frame.frameKey,
        renderGridLayer({
            frameResult: frame,
    ...
    const state = {
        ...
        currentRendered: orderedFrames.length > 0
