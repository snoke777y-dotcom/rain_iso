const MAGIC = "RTA1";
export function encodeNamedTypedArrays(fields) {
    const payloadFields = [];
    const rawChunks = [];
    let byteOffset = 0;
    for (const [name, array] of Object.entries(fields)) {
        const chunk = new Uint8Array(array.buffer.slice(array.byteOffset, array.byteOffset + array.byteLength));
        payloadFields.push({
            name,
...
export function decodeNamedTypedArrays(bytes) {
    ...
    const payload = JSON.parse(new TextDecoder().decode(buffer.subarray(payloadStart, payloadEnd)));
    const result = {};
    const rawBase = payloadEnd;
    for (const field of payload.fields) {
        const rawSlice = buffer.slice(rawBase + field.byteOffset, rawBase + field.byteOffset + field.byteLength);
...
function createTypedArray(type, buffer, length) {
    switch (type) {
        case "Int32Array":
...
function normalizeToUint8Array(bytes) {
