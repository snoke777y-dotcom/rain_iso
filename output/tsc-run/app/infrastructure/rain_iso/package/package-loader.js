...
import { buildRainIsoSequenceFromApiResponse } from "./raw-api-adapter.js";
import { PackageValidationError, validateLoadedRainIsoPackage, validateRawPackage } from "./package-validator.js";
export async function loadRainIsoPackage(options) {
    ...
    const rain5m = buildRainIsoSequenceFromApiResponse(rawPackage.realtime5mResponse, {
        productType: FrameType.Rain5m
    });
    const accum1h = buildRainIsoSequenceFromApiResponse(rawPackage.realtime1hResponse, {
...
async function readRawRainApiResponse(filePath) {
    try {
        ...
        throw new PackageValidationError(error instanceof Error ? error.message : "无法读取原始接口文件");
    }
}
